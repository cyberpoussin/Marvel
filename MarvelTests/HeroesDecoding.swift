...
class HeroesDecoding: XCTestCase {
    var heroes: [Hero] = []
    ...
    private func mockData(name: String) -> Data {
        guard
            let mockFixturesURL = Bundle.main.url(forResource: name, withExtension: "json"),
    ...
    func testCharactersRequestResponseDecoding() throws {
        let data = mockData(name: "Heroes")
        let decoder = JSONDecoder()
        let decodedResponse = try decoder.decode(CharactersRequestResponse.self, from: data)
        XCTAssertEqual(heroes, decodedResponse.data.results)
    }
    
    func testHeroesEncoding() throws {
        let encoder = JSONEncoder()
        let data = try encoder.encode(heroes)
        let decoder = JSONDecoder()
        let newHeroes = try decoder.decode([Hero].self, from: data)
        XCTAssertEqual(heroes, newHeroes)
    }
}
